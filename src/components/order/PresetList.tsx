...
import type { MealTypeId, PresetMeal } from "../../data/types";
import { itemPrice, macros } from "../../lib/pricing";
import { MealImage } from "../MealImage";

export function PresetList({ meals, buildType }: { meals: PresetMeal[]; buildType: MealTypeId }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {meals.map((meal) => {
        ...
        return <Link key={meal.id} href={`/meal/${meal.id}`} className="flex min-h-28 gap-3 overflow-hidden rounded-2xl border border-line bg-card p-3 shadow-sm"><div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-xl"><MealImage src={meal.image} alt={meal.name} fallbackLetter={meal.name} /></div><div className="min-w-0 flex-1 py-1"><div className="flex items-start justify-between gap-2"><h2 className="font-display text-[18px] leading-tight font-extrabold tracking-[-0.02em] text-ink">{meal.name}</h2><span className="shrink-0 text-[15px] font-semibold tabular-nums text-cardinal">${itemPrice(selection).toFixed(2)}</span></div><p className="mt-1 line-clamp-2 text-[13px] leading-5 text-ink-soft">{meal.blurb}</p><p className="mt-2 text-[13px] font-semibold text-ink-soft">{nutrition.calories} cal · {nutrition.protein}g protein</p></div></Link>;
      })}
    </div>
  );
}
